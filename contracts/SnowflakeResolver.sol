...
contract SnowflakeResolver is Ownable {
    ...
    // this can be overriden to initialize other variables, such as e.g. an ERC20 object to wrap the PHNX token
    ...
    function transferPhoenixBalanceTo(uint einTo, uint amount) internal {
        PhoenixInterface phoenix = PhoenixInterface(SnowflakeInterface(snowflakeAddress).phoenixTokenAddress());
        require(phoenix.approveAndCall(snowflakeAddress, amount, abi.encode(einTo)), "Unsuccessful approveAndCall.");
    }
    ...
    function transferPhoenixBalanceToVia(address via, uint einTo, uint amount, bytes memory snowflakeCallBytes) internal {
        PhoenixInterface phoenix = PhoenixInterface(SnowflakeInterface(snowflakeAddress).phoenixTokenAddress());
        require(
    ...
    function withdrawPhoenixBalanceToVia(address via, address to, uint amount, bytes memory snowflakeCallBytes) internal {
        PhoenixInterface phoenix = PhoenixInterface(SnowflakeInterface(snowflakeAddress).phoenixTokenAddress());
        require(
            phoenix.approveAndCall(
